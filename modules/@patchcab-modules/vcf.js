...
import { Faceplate, Knob, Patch, Switch } from '/js/core.js';

/* src/VCF.svelte generated by Svelte v3.31.2 */
...
function create_default_slot(ctx) {
	...
	if (/*state*/ ctx[0].cv !== void 0) {
		knob1_props.value = /*state*/ ctx[0].cv;
	}
	...
	return {
		...
		p(ctx, dirty) {
			...
			if (!updating_value_1 && dirty & /*state*/ 1) {
				...
				knob1_changes.value = /*state*/ ctx[0].cv;
			...
			if (dirty & /*filter*/ 2) patch1_changes.input = /*filter*/ ctx[1];
			patch1.$set(patch1_changes);
			const patch2_changes = {};
			if (dirty & /*filter*/ 2) patch2_changes.output = /*filter*/ ctx[1];
...
function create_fragment(ctx) {
	...
	return {
		...
		p(ctx, [dirty]) {
			...
			if (dirty & /*$$scope, filter, state, scale*/ 263) {
...
function instance($$self, $$props, $$invalidate) {
	let { state = { freq: 440, type: "lowpass", cv: 1 } } = $$props;
	const filter = new Tone.Filter(state.freq, state.type);
	...
	const onConnect = nodes => {
		if (nodes) {
			...
			$$invalidate(1, filter.frequency.overridden = false, filter);
			$$invalidate(1, filter.frequency.value = state.freq, filter);
		}
	};
	...
	$$self.$$.update = () => {
		...
		if ($$self.$$.dirty & /*state*/ 1) {
			 $$invalidate(1, filter.type = state.type, filter);
		}

		if ($$self.$$.dirty & /*state*/ 1) {
			 $$invalidate(2, scale.min = Math.max(MIN, state.freq - state.freq * state.cv), scale);
		}

		if ($$self.$$.dirty & /*state*/ 1) {
			 $$invalidate(2, scale.max = Math.min(MAX, state.freq + (MAX - state.freq) * state.cv), scale);
		}
	};
...
class VCF extends __sv.SvelteComponent {
	constructor(options) {
		super();
