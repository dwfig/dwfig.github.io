...
/* src/FM.svelte generated by Svelte v3.31.2 */
...
function instance($$self, $$props, $$invalidate) {
	...
	const oscillator = new Tone.FMOscillator(state.freq, state.type).start();
	const scale = new Tone.Scale(0, MAX_MOD);
	...
	const onConnect = nodes => {
		if (nodes) {
			...
			$$invalidate(1, oscillator.modulationIndex.overridden = false, oscillator);
			$$invalidate(1, oscillator.modulationIndex.value = 3, oscillator);
		}
	};
	...
	$$self.$$.update = () => {
		...
		if ($$self.$$.dirty & /*state*/ 1) {
			 $$invalidate(2, scale.max = MAX_MOD * state.fm, scale);
		}
	};
...
class FM extends __sv.SvelteComponent {
	constructor(options) {
		super();
